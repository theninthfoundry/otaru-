...
export default function CheckoutPage() {
  ...
  // Stable idempotency key per checkout session (regenerated when page mounts)
  ...
  const handleRazorpayPayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If mock or no SDK loaded, fall into sandbox mode automatically
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response: any) => {
          // Verify on server — include nonce for replay protection
      ...
      if (step < 3) {
        ...
        setTimeout(async () => {
          if (sandboxOutcome === 'success') {
            // If we don't have an orderId yet, create one
