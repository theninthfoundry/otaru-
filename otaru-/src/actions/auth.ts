...
/**
 * Log in patron, store session in an HTTP-only secure cookie, and redirect.
 */
export async function loginPatron(prevState: { error?: string } | null, formData: FormData) {
  ...
  // Pre-seed some default demo accounts or allow any valid inputs for high accessibility
...
/**
 * Retrieve session from cookie (server-side).
 */
