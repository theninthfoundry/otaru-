...
export function addTransaction(tx: Omit<Transaction, 'createdAt' | 'history'> & { details?: string }): Transaction {
  ...
  // If order already exists (e.g. updating order from CREATED to CAPTURED), update it instead
  const existingIdx = transactions.findIndex(t => t.orderId === tx.orderId);
  if (existingIdx !== -1) {
    const existing = transactions[existingIdx];
    // Keep history and append new event
    existing.status = tx.status;
    existing.id = tx.id; // Update payment id if sandbox/real was assigned
    existing.history.push({
      status: tx.status,
      timestamp,
      details: tx.details || `Transaction updated to status: ${tx.status}`,
    });
    // Merge security verification status
    existing.security = {
      ...existing.security,
      ...tx.security,
    };
...
export function getStats(): LedgerStats {
  ...
  // Rate limits can be calculated from failed security logs or blocks
