...
const components: PortableTextComponents = {
  ...
  marks: {
    strong: ({ children }: any) => <strong className="font-semibold text-otaru-ink">{children}</strong>,
    ...
    artifactRef: ({ children, value }: any) => (
      <a
  ...
  types: {
    ...
    imageMasonry: ({ value }: any) => {
      const mappedImages = (value?.images ?? []).map((img: any) => ({
        url: img?.asset?.url || img?.url || '',
        alt: img?.alt ?? '',
        caption: img?.caption,
      }));
      return <ImageMasonry images={mappedImages} title={value?.title} />;
    },
    materialCallout: ({ value }: any) => (
